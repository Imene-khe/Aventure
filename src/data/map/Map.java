...
public class Map {
    ...
    private HashMap<Block, Obstacle> obstacles;
    private HashMap<Block, Boolean> terrainBlocked;
    private HashMap<Block, String> staticObjects;
    private HashMap<Block, String> staticTerrain;
    private HashMap<Block, String> enemies;
    ...
    public Map(int lineCount, int columnCount) {
    	...
    	this.terrainBlocked= new HashMap<>();
    	this.staticObjects= new HashMap<>();
    	this.staticTerrain= new HashMap<>();
            ...
            for (int columnIndex = 0; columnIndex < columnCount; columnIndex++) {
                ...
                if (terrainType.equals("grass")) {
                    ...
                    if (rand < 0.05) {
                    	...
                    	staticObjects.put(block, "chest");  // Déclare un coffre sur ce bloc
                        chestManager.addChest(block, "chest");  // Ajoute le coffre à ChestManager
                        setTerrainBlocked(block, true);  // Bloque le terrain pour ce bloc

                        System.out.println("Coffre créé à la position (" + lineIndex + ", " + columnIndex + ")");
                    }
                }
